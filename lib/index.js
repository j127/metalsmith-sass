  ...
  var sass = require("node-sass"),
    ...
    compile = function (config, basePath, files, filename, done) {
      ...
      var file = files[filename],
        ...
        computedOutputDir =
          typeof opts.outputDir === "function"
            ? opts.outputDir(path.dirname(filename))
            : opts.outputDir,
        dest = path.join(
          computedOutputDir,
          path.basename(filename).replace(/\.s[ca]ss/, ".css")
        );
    ...
    compileSass = function compileSass(config, files, metalsmith, done) {
      ...
      var directory = metalsmith.dir || metalsmith._directory,
        ...
        basePath = path.isAbsolute(source)
          ? source
          : path.join(directory, source);
      each(
        Object.keys(files),
        compile.bind(null, config, basePath, files),
        done
      );
    },
